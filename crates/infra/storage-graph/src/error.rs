...
/// Errors returned by the graph storage engine.
...
pub enum Error {
    ...
    /// A requested node, edge, or property was not found.
    #[error("not found: {0}")]
    NotFound(String),
    ...
    /// A property map could not be serialized or deserialized.
    #[error("property encoding error: {0}")]
    PropertyEncoding(String),

    /// An index entry is missing or inconsistent.
    #[error("index error: {0}")]
    Index(String),
}
