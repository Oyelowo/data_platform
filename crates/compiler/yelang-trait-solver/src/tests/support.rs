...
use yelang_ty::ty::{AdtDef, ImplPolarity, ProjectionTy, Ty, TyKind};
...
pub struct TestCtxt<'tcx> {
    ...
    trait_assoc_items: FxHashMap<DefId, Vec<AssocItemInfo<'tcx>>>,
    impl_assoc_items: FxHashMap<DefId, Vec<AssocItemInfo<'tcx>>>,
    adt_fields: FxHashMap<DefId, Vec<Ty<'tcx>>>,
}
...
impl<'tcx> TestCtxt<'tcx> {
    ...
    pub fn add_trait_supertraits(&mut self, def_id: DefId, supertraits: Vec<TraitRef<'tcx>>) {
    ...
    pub fn add_trait_assoc_item(&mut self, trait_def_id: DefId, item: AssocItemInfo<'tcx>) {
    ...
    pub fn add_impl_assoc_item(&mut self, impl_def_id: DefId, item: AssocItemInfo<'tcx>) {
    ...
    pub fn set_adt_fields(&mut self, adt_def_id: DefId, fields: Vec<Ty<'tcx>>) {
    ...
    fn projection_ty(&self, trait_def_id: DefId, item_def_id: DefId, self_ty: Ty<'tcx>) -> ProjectionTy<'tcx> {
        let args = self
            .interner
    ...
    pub fn mk_adt(&self, def_id: DefId, args: &[Ty<'tcx>]) -> Ty<'tcx> {
        let args: Vec<_> = args
            .iter()
            .map(|&ty| yelang_ty::generic::GenericArg::Type(ty))
            .collect();
        self.interner
            .mk_ty(TyKind::Adt(AdtDef { def_id }, self.interner.mk_generic_args(&args)))
    }
    ...
    pub fn mk_pair(&self, a: Ty<'tcx>, _b: Ty<'tcx>) -> Ty<'tcx> {
        let args = self
            .interner
            .mk_generic_args(&[yelang_ty::generic::GenericArg::Type(a)]);
        let inner = self.interner.mk_ty(TyKind::Adt(
