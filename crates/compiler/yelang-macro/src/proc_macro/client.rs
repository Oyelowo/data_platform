...
use yelang_proc_macro_bridge::protocol::{
    CURRENT_PROTOCOL_VERSION, ErrorCode, LibraryHandle, Request, Response, WireTokenStream,
...
impl ProcMacroClient {
    ...
    /// Load a proc-macro dynamic library by path.
    pub fn load_library(&mut self, path: &str) -> Result<LibraryHandle, ProcMacroClientError> {
        self.send_request(&Request::LoadLibrary {
            path: path.to_string(),
        })?;
        match self.read_response()? {
            Response::LibraryLoaded { library, .. } => Ok(library),
            Response::Error { code, message } => Err(ProcMacroClientError::Server(code, message)),
            other => Err(ProcMacroClientError::Protocol(format!(
    ...
    ) -> Result<WireTokenStream, ProcMacroClientError> {
    ...
    fn read_expanded(&mut self) -> Result<WireTokenStream, ProcMacroClientError> {
        loop {
            match self.read_response()? {
                Response::Expanded { output } => return Ok(output),
                Response::Diagnostic { .. } => {
                    // TODO: collect diagnostics and surface them.
                    continue;
                }
