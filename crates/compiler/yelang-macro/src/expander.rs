...
use crate::cfg::{CfgOptions, eval_cfg_attr_attribute, eval_cfg_attribute};
use crate::eager::{
    EagerBuiltin, EagerContext, EnvProvider, FileLoader, StdEnvProvider, StdFileLoader,
...
use yelang_proc_macro_bridge::protocol::token::WireSpan;
...
pub struct MacroExpander<'a> {
    ...
    /// Current module path, used for scoped declarative-macro resolution.
...
impl<'a> MacroExpander<'a> {
    ...
    pub fn expand_item(&mut self, item: Item) -> Result<Vec<Item>, ExpandError> {
        let Some((primary, side_items)) = self.expand_item_attributes(item)? else {
            return Ok(Vec::new());
        };
    ...
    ///
    /// Returns `Ok(None)` when the item is removed by a `#[cfg(false)]` predicate.
    fn expand_item_attributes(
        &mut self,
        mut item: Item,
    ) -> Result<Option<(Item, Vec<Item>)>, ExpandError> {
        ...
        while let Some(attr) = item.attributes.first().cloned() {
            ...
            // `#[cfg(...)]` and `#[cfg_attr(...)]` are processed before any other
            // attributes because they control whether the item (and the attributes
            // it carries) exist at all.
            if attr_name == "cfg" {
                match eval_cfg_attribute(
                    &attr,
                    &self.cfg_options,
                    self.interner,
                    &self.eager_context(),
                    ...
                    Err(e) => {
                        self.errors.push(e.with_backtrace(self.backtrace()));
                        return Ok(None);
                    }
                }
            }
    ...
    fn expand_item_deep(&mut self, mut item: Item) -> (Vec<Item>, bool) {
        ...
        match &mut item.kind {
            ...
            ItemKind::Module(m) => {
                let mod_name = self.interner.resolve(&m.name.symbol).to_string();
    ...
    fn is_user_attribute_macro(&self, attr: &Attribute) -> bool {
        ...
        if self
            .resolver
            .resolve(name_str, &self.module_path)
    ...
    fn expand_user_attribute_macro(
            ...
            && mac.kind() == yelang_proc_macro_bridge::protocol::ProcMacroKind::Attribute
        {
            ...
            let def_site = mac.def_site_span();
            let result = self.with_proc_macro_sites(span, def_site, |this| {
                this.expand_in_process_attr(mac, proc_args, proc_item, span)
            });
            self.after_expand();
        ...
        let mac = self.resolver.resolve(&name_str, &self.module_path)?.clone();
    ...
    fn expand_user_derive_macro(
            ...
            && mac.kind() == yelang_proc_macro_bridge::protocol::ProcMacroKind::Derive
        {
            ...
            let def_site = mac.def_site_span();
            let result = self.with_proc_macro_sites(span, def_site, |this| {
                this.expand_in_process_derive(mac, proc_item, span)
            });
            self.after_expand();
            return Some(match result {
                Ok(stream) => parse_macro_output::parse_items_from_macro_stream(
        ...
        let mac = self
            .resolver
            .resolve(trait_name, &self.module_path)?
            .clone();
        let rules: Vec<&crate::matcher::types::MacroRule> = mac
            .rules
            .iter()
    ...
    fn push_proc_macro_diagnostics(
        ...
        for diag in diagnostics {
            let level = match diag.level {
                yelang_proc_macro::Level::Error => crate::error::DiagnosticLevel::Error,
                yelang_proc_macro::Level::Warning => crate::error::DiagnosticLevel::Warning,
                yelang_proc_macro::Level::Note => crate::error::DiagnosticLevel::Note,
                yelang_proc_macro::Level::Help => crate::error::DiagnosticLevel::Help,
            };
            self.errors.push(
    ...
    fn expand_server_fn_like(
        ...
        let (def_site, mixed_site) = self.proc_macro_sites(span, mac.def_site_span);
        let runtime = self
            .proc_macro_runtime
            .as_ref()
    ...
    /// invocation.
    ///
    /// `def_site` uses the macro-definition span when one is available; it falls
    /// back to the call site for macros whose definition location is unknown.
    /// `mixed_site` is a fresh syntax context with mixed transparency parented
    /// to the call-site context.
    fn proc_macro_sites(
        &mut self,
        span: yelang_lexer::Span,
        def_site_span: Option<yelang_lexer::Span>,
    ) -> (yelang_lexer::Span, yelang_lexer::Span) {
        let def_site = def_site_span.unwrap_or(span);
    ...
    /// Run a closure with the proc-macro API's call/def/mixed site spans set to
    /// the real invocation spans, then clear them afterwards.
    fn with_proc_macro_sites<R>(
        &mut self,
        span: yelang_lexer::Span,
        def_site_span: Option<yelang_lexer::Span>,
        f: impl FnOnce(&mut Self) -> R,
    ) -> R {
        let (def_site, mixed_site) = self.proc_macro_sites(span, def_site_span);
        yelang_proc_macro::bridge::set_call_site_from_wire(span_to_wire(span));
        yelang_proc_macro::bridge::set_def_site_from_wire(span_to_wire(def_site));
        yelang_proc_macro::bridge::set_mixed_site_from_wire(span_to_wire(mixed_site));
        let result = f(self);
        yelang_proc_macro::bridge::clear_sites();
        result
    }
    ...
    fn expand_macro_invocation(
            ...
            && let Some(mac) = executor.find(&name)
        {
            ...
            let def_site = mac.def_site_span();
            let result = self.with_proc_macro_sites(span, def_site, |this| {
                this.expand_in_process_fn_like(mac, &macro_args, span)
            });
            self.after_expand();
            return result;
        }
        ...
        let Some(mac) = self.resolver.resolve(&name, &self.module_path) else {
...
mod tests {
    ...
    #[test]
    fn cfg_true_keeps_item_and_removes_attribute() {
        let src = r#"
            @cfg(test)
            fn feature_fn() {}
        "#;
        let (program, interner) = parse_program(src);
        let mut expander =
            MacroExpander::new(&interner).with_cfg_options(CfgOptions::new().with_name("test"));
        let result = expander.expand(&program);
        assert!(result.errors.is_empty(), "errors: {:?}", result.errors);
    ...
    #[test]
    fn cfg_attr_true_injects_attribute() {
        // @test on a function is valid; when injected by cfg_attr it should be
    ...
    #[test]
    fn cfg_attr_false_does_not_inject_attribute() {
        // @test on a struct would error, so if cfg_attr(false) incorrectly
        // injected it expansion would fail.
        let src = r#"
            @cfg_attr(missing_feature, test)
            struct SafeStruct {}
        "#;
        let (program, interner) = parse_program(src);
        let mut expander =
        ...
        assert!(result.errors.is_empty(), "errors: {:?}", result.errors);
        assert_eq!(result.program.items.len(), 1);
        assert!(matches!(result.program.items[0].kind, ItemKind::Struct(_)));
    }

    #[test]
    fn cfg_attr_can_inject_cfg_false() {
        let src = r#"
            @cfg_attr(test, cfg(missing_feature))
            fn removed_fn() {}
        "#;
        let (program, interner) = parse_program(src);
        let mut expander =
    ...
    #[test]
    fn paste_in_declarative_macro_generates_identifier() {
        let src = r#"
            macro make_fn {
                ($name:ident) => {
                    fn paste!([<foo _ $name>])() {}
        ...
        let fn_item = result
            ...
            .expect("expected generated fn");
        let ItemKind::Fn(func) = &fn_item.kind else {
            unreachable!()
        };
        assert_eq!(
            interner.resolve(&func.name.symbol),
            "foo_bar",
            "paste! should concatenate identifier fragments"
        );
    }

    #[test]
    fn private_module_macro_not_visible_at_root() {
        let src = r#"
            mod inner {
                macro local { () => { fn f() {} }; }
            }
        ...
        assert!(
            result
                ...
                .iter()
                .any(|e| matches!(e, ExpandError::UnknownMacro { .. })),
            "expected unknown macro error for private module macro, got {:?}",
            result.errors
        );
    }

    #[test]
    fn public_module_macro_visible_at_root() {
        let src = r#"
            mod inner {
                pub macro exported { () => { fn f() {} }; }
            }
            exported!()
        "#;
        let (program, interner) = parse_program(src);
        ...
        assert!(
            result
                ...
                .items
                .iter()
                .any(|i| matches!(i.kind, ItemKind::Fn(_)))
        );
    }

    #[test]
    fn root_macro_visible_in_child_module() {
        let src = r#"
            macro outer { () => { fn f() {} }; }
            mod inner {
                outer!()
