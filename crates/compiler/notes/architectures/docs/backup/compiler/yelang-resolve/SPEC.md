...
- `yelang-arena` (path: `../yelang-arena`) — for FxHashMap, OrderedMap, DefId
- `thiserror` (workspace)
...
6. **Use yelang-arena wrappers**: `FxHashMap`, `FxHashSet`, `OrderedMap` from `yelang-arena`.
7. **Use yelang-arena Id types**: `DefId` from `yelang_arena::id`.
