...
use crate::{
    Any, ByteLexed, CarriageReturn, Char, CharCursor, CharLexerError, CharLexerResult, Comment,
    DatetimeLexed, DurationLexed, Either, FileId, FloatLexed, Geometry, IdentLexed, IntLexed,
    OneOf3, Position, RecordIdLexed, Repeat, Span, StringLitLexed, Tab, Token, TokenError,
    TokenStream, TokenStreamBuilder, UuidLexed, Whitespace, try_parse, word::Word2,
};
    ...
    fn next_token(
        ...
        if cursor.peek() == Some('\'') {
            ...
            if let Ok(ident) = cursor.parse::<IdentLexed>() {
                ...
                if cursor.peek() == Some('\'') {
                    ...
                    return Ok(Some(Token::<crate::tokenizer::TokenKind>::new(
        ...
        if let Ok(ident) = cursor.parse::<IdentLexed>() {
            ...
            let kind = match text {
                ...
                "hops" => TokenKind::HopsKw,
                "enumerate" => TokenKind::Enumerate,
        ...
        let token_kind = try_parse!(
            ...
            cursor
                .parse::<(FloatLexed, crate::not::PeekNot<Char<'.'>>)>()
                .map(
                    |(f, _): (FloatLexed, crate::not::PeekNot<Char<'.'>>)| TokenKind::Lit(
        ...
        Ok(Some(Token::<crate::tokenizer::TokenKind>::new(
            token_kind,
            cursor.span_since(start_pos),
        )))
    }
