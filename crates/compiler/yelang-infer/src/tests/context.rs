...
fn eq_const_unifies_infer_vars() {
    ...
    let a = infcx.new_const_var(t_i32);
    let b = infcx.new_const_var(t_i32);
    assert!(infcx.eq_const(a, b).is_ok());
}
...
fn eq_const_unifies_infer_with_value() {
    ...
    let mut infcx = InferCtxt::new();
    let t_i32 = interner.mk_ty(TyKind::Int(IntTy::I32));
    let var = infcx.new_const_var(t_i32);
    let value = Const {
        kind: ConstKind::Value(ConstValue::Int(42)),
        ty: t_i32,
    };
    assert!(infcx.eq_const(var, value).is_ok());
    // A second variable unified with the same value should also succeed.
    let var2 = infcx.new_const_var(t_i32);
    assert!(infcx.eq_const(var2, value).is_ok());
}
...
fn eq_const_rejects_value_mismatch() {
    ...
    let t_i32 = interner.mk_ty(TyKind::Int(IntTy::I32));
    let a = Const {
        kind: ConstKind::Value(ConstValue::Int(1)),
        ty: t_i32,
    };
    let b = Const {
        kind: ConstKind::Value(ConstValue::Int(2)),
        ty: t_i32,
    };
    match infcx.eq_const(a, b) {
...
fn eq_int_var_rejects_mismatch() {
    ...
    let t_i32 = interner.mk_ty(TyKind::Int(IntTy::I32));
    let t_i64 = interner.mk_ty(TyKind::Int(IntTy::I64));
    assert!(infcx.eq(var, t_i32).is_ok());
    match infcx.eq(var, t_i64) {
...
fn eq_float_var_rejects_mismatch() {
    ...
    let t_f32 = interner.mk_ty(TyKind::Float(FloatTy::F32));
    let t_f64 = interner.mk_ty(TyKind::Float(FloatTy::F64));
    assert!(infcx.eq(var, t_f32).is_ok());
    match infcx.eq(var, t_f64) {
...
fn eq_placeholder_equal() {
    ...
    let a = interner.mk_ty(TyKind::Placeholder(p));
    let b = interner.mk_ty(TyKind::Placeholder(p));
    assert!(infcx.eq(a, b).is_ok());
}
...
fn eq_placeholder_different_rejects() {
    let interner = Interner::new();
    let mut infcx = InferCtxt::new();
    let a = interner.mk_ty(TyKind::Placeholder(PlaceholderType {
        universe: UniverseIndex(0),
        name: Symbol::from(1),
    }));
    let b = interner.mk_ty(TyKind::Placeholder(PlaceholderType {
        universe: UniverseIndex(0),
        name: Symbol::from(2),
    }));
    match infcx.eq(a, b) {
...
fn eq_bound_var_equal() {
    ...
    let a = interner.mk_ty(TyKind::Bound(DebruijnIndex::INNERMOST, bound));
    let b = interner.mk_ty(TyKind::Bound(DebruijnIndex::INNERMOST, bound));
    assert!(infcx.eq(a, b).is_ok());
}
...
fn eq_bound_var_different_rejects() {
    let interner = Interner::new();
    let mut infcx = InferCtxt::new();
    let a = interner.mk_ty(TyKind::Bound(
    ...
    let b = interner.mk_ty(TyKind::Bound(
        DebruijnIndex::INNERMOST,
        BoundTy {
            var: BoundVar(1),
            kind: BoundTyKind::Anon,
...
fn eq_generic_arg_kind_mismatch() {
    ...
    match infcx.eq(
        interner.mk_ty(TyKind::Tuple(args_a)),
        interner.mk_ty(TyKind::Tuple(args_b)),
    ) {
...
fn eq_projection_trait_ref_mismatch() {
    ...
    let proj_a = interner.mk_ty(TyKind::Projection(ProjectionTy {
        trait_ref: trait_a,
        item_def_id: DefId::new(10),
    }));
    let proj_b = interner.mk_ty(TyKind::Projection(ProjectionTy {
        trait_ref: trait_b,
        item_def_id: DefId::new(10),
    }));
    match infcx.eq(proj_a, proj_b) {
