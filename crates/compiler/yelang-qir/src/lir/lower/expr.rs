...
use crate::lir::lower::method::{lower_method_call, LoweredMethod};
...
pub fn lower_hir_expr(
    ...
    match expr {
        ...
        Expr::Query(query_id) => {
            ...
            let lir_id = super::query::lower_query(plan, ctx, &query)?;
            // A subquery expression evaluates to the result of the plan.
            Ok(plan.alloc_expr(QExpr::Subplan(lir_id, ty)))
        }
        Expr::Comprehension { .. } => {
            let lir_id = super::selector::lower_comprehension(plan, ctx, expr_id)?;
            Ok(plan.alloc_expr(QExpr::Subplan(lir_id, ty)))
        }
